...
class ModelEvictCacheAspect extends AbstractAspect
{
    ...
    public function process(ProceedingJoinPoint $proceedingJoinPoint)
    {
        try {
            ...
            $key = $this->modelCacheService->getCacheableValue(ModelEvictCache::class, $className, $method, $arguments);
    ...
    public function updateCache($proceedingJoinPoint): bool
    {
        ...
        $pkColumnValue = $arguments['pkColumnValue'] ?? '';
        $subPkColumnValue = $arguments['subPkColumnValue'] ?? '';
        $ttl = $arguments['ttl'] ?? 0;
        ...
        $key = $this->modelCacheService->getCacheableValue(ModelEvictCache::class, $className, $method, $arguments);
        $group = $this->config->get('model_cache.redis_select', 'default');
        $delKey = $subKey ? $subPkColumnValue : $pkColumnValue;
        $this->modelCacheService->delCache($key, $delKey, $isList, $group, $useContext);
